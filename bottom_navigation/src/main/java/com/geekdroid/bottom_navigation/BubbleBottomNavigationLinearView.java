...
public class BubbleBottomNavigationLinearView extends LinearLayout implements View.OnClickListener, IBubbleNavigation {
    ...
    private ArrayList<BubbleBottomToggleView> bubbleNavItems;
    private BubbleBottomNavigationChangeListener navigationChangeListener;
     ...
     * Constructors
     */
    public BubbleBottomNavigationLinearView(@NonNull Context context) {
        super(context);
        init(context, null);
    }

    public BubbleBottomNavigationLinearView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public BubbleBottomNavigationLinearView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    /**
     * Finds Child Elements of type {@link BubbleBottomToggleView} and adds them to {@link #bubbleNavItems}
     */
    private void updateChildNavItems() {
        bubbleNavItems = new ArrayList<>();
        for (int index = 0; index < getChildCount(); ++index) {
            View view = getChildAt(index);
            if (view instanceof BubbleBottomToggleView)
                bubbleNavItems.add((BubbleBottomToggleView) view);
            else {
    ...
    private void updateMeasurementForItems() {
        ...
        if (numChildElements > 0) {
            ...
            for (BubbleBottomToggleView btv : bubbleNavItems)
    ...
    @Override
    public void setNavigationChangeListener(BubbleBottomNavigationChangeListener navigationChangeListener) {
    ...
    public void setCurrentActiveItem(int position) {
        ...
        BubbleBottomToggleView btv = bubbleNavItems.get(position);
        btv.performClick();
    }
    ...
    public void onClick(View v) {
        ...
        if (changedPosition >= 0) {
            ...
            BubbleBottomToggleView currentActiveToggleView = bubbleNavItems.get(currentActiveItemPosition);
            BubbleBottomToggleView newActiveToggleView = bubbleNavItems.get(changedPosition);
